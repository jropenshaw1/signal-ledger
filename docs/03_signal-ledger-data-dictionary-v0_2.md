# Signal Ledger — Data Dictionary v0.2

**Status:** LOCKED — 2026-05-03
**Version:** v0.2
**LENS Chain Position:** Layer 3 of 7
**Depends On:** Charter v0.2 (LOCKED 2026-05-03), Use Case Spec v0.1 (LOCKED 2026-04-29)
**Precedes:** Functional Spec v1.0 (re-validated, no re-lock needed)
**Supersedes:** Data Dictionary v0.1 (LOCKED 2026-04-30)
**Tag:** [signal-ledger:data-dictionary-v0.2]

---

## Changes from v0.1

This version applies the backward-flow correction mandated by LENS Governance Addendum v0.1 (principle G4). Defects surfaced during the post-Functional-Spec structural diagram review and re-validation pass.

### Schema additions

| Change | Reason |
|---|---|
| **E7 added** — `sl_capture_events` table | Referenced extensively in Functional Spec § 3 (capture event semantics) but absent from DD v0.1. Surfaced by structural diagram review. |
| **E8 added** — `sl_signpost_embeddings` table | Referenced in Functional Spec § 2 (embedding pipeline) but absent from DD v0.1. The vector column previously co-located on `sl_articles` was an inconsistency between layers. Embeddings now live in a dedicated table with unique-constraint idempotency per Functional Spec. |
| **`embedding_status` field added to E1** | Lifecycle field defined in Functional Spec § 2, not present in DD v0.1. Values: `pending` \| `processing` \| `complete` \| `failed`. |
| **`embedding` field removed from E1** | Embeddings live in E8 (`sl_signpost_embeddings`), not on the article row. v0.1 had them on `sl_articles`; Functional Spec § 2 placed them in a separate table. v0.2 aligns to Functional Spec. |
| **`external_id` field added to E1** | Functional Spec § 5 declares idempotency key `(provider_id, external_id)` and a unique constraint on `sl_articles`. DD v0.1 did not define this column, leaving the constraint unenforceable. v0.2 adds it. |

### Vocabulary corrections (drift between DD v0.1 and Functional Spec)

| Field | DD v0.1 | DD v0.2 (matches Functional Spec § 7) | Reason |
|---|---|---|---|
| Gap window field name | `gap_start` | `start_date` | Functional Spec § 7 vocabulary |
| Gap window field name | `gap_end` | `end_date` | Functional Spec § 7 vocabulary |
| Gap window field name | `resolution_status` | `status` | Functional Spec § 7 vocabulary |
| Gap window enum values | `pending` \| `resolved` \| `unresolvable` (3 states) | `open` \| `acknowledged` \| `resolved` \| `unresolvable` (4 states) | Functional Spec § 7 introduces `acknowledged` as the manual-triage-in-progress state. 4-state lifecycle is the implementation truth. |
| Gap window object additions | — | `provider_id`, `resolution_notes`, `last_updated` per Functional Spec § 7 JSONB schema | Functional Spec § 7 specifies these fields for optimistic-locking and audit. |

### Visual artifacts (per Governance Addendum G1)

| Artifact | Coverage |
|---|---|
| ER diagram | All 8 entities (E1–E8) with cardinality and FK relationships. JSONB sub-records (E1a, E1b, E6) shown as embedded structures. |
| State machine — E1 `embedding_status` | Article embedding lifecycle. |
| State machine — E6 gap window `status` | Gap window resolution lifecycle. |

### LENS chain position

| Field | v0.1 | v0.2 |
|---|---|---|
| LENS chain position | Layer 3 of 6 | Layer 3 of 7 (Threat Model added as Layer 5 by Governance Addendum v0.1) |

### Re-validation against Functional Spec v1.0

Functional Spec v1.0 references `sl_capture_events`, `sl_signpost_embeddings`, and `embedding_status` correctly throughout. With DD v0.2 now defining these entities, Functional Spec v1.0 requires **no re-lock**. The drift was unidirectional — DD lagged Functional Spec; Functional Spec did not lag itself. This re-validation is the documented record per the handoff decision.

---

## Storage Architecture (unchanged from v0.1)
...
**Note on `embedding` field:** v0.1 placed an `embedding vector(1536)` column directly on `sl_articles`. v0.2 removes it. Embeddings live in E8 (`sl_signpost_embeddings`) per Functional Spec § 2. This is the single biggest schema-shape change between v0.1 and v0.2.
...
**Vocabulary aligned to Functional Spec § 7 in v0.2.** Field names changed: `gap_start` → `start_date`, `gap_end` → `end_date`, `resolution_status` → `status`. Lifecycle expanded from 3 states to 4 (added `acknowledged`). Three additional fields added per Functional Spec JSONB schema: `provider_id`, `resolution_notes`, `last_updated`.
...
| `metadata` | JSONB | ❌ | Free-form object or null | Extension point for event-specific context not covered by the columns above (e.g., HTTP status codes, embedding-API response IDs). Optional. | A2 |
...
**Traceability requirement:** Given only `sl_capture_events` and an `article_id`, a full ingestion timeline must be reconstructible. If field set above is insufficient to satisfy this, the event schema is incomplete and a v0.3 backward-flow correction is required.
...
**Note:** The HNSW vector index moved from `sl_articles.embedding` (v0.1) to `sl_signpost_embeddings.embedding` (v0.2) when embeddings relocated to E8.
...
| OQ-DD-02 | `body_text` storage — large article bodies in Postgres text column | **Closed in v0.2** — Postgres text column confirmed sufficient at v1 corpus scale (≤ a few thousand articles). Revisit if a single article body exceeds 1MB compressed; not expected. |
...
| OQ-DD-06 | E7 `metadata` field shape — should it be schema-constrained per `event_type` or remain free-form JSONB? | **Open** — defer until observability instrumentation reveals which fields are consistently populated. Free-form is the v1 default; schema constraint can be added in v0.3 if patterns stabilize. |
| OQ-DD-07 | E8 `provenance` field shape — recommended fields are unstructured; should this become a typed sub-record? | **Open** — defer until first production use surfaces what the embedding API actually returns at scale. Free-form JSONB is the v1 default. |
...
*Signal Ledger Data Dictionary v0.2 — LOCKED 2026-05-03. Supersedes v0.1.*
